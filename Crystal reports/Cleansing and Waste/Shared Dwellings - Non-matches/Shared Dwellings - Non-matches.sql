...
(
  ...
  on pbr.Property_Key_LINK_TO_PROPERTY = parent_properties.parent_key
	...
	on bin_requests.PropertyKey = rates.Parent_Property_TPKLPAPROP
	left join
	(
	...
	on rates_tasks.PropertyKey = rates.Parent_Property_TPKLPAPROP
	left join
	(
	...
	on cw_tasks.PropertyKey = rates.Parent_Property_TPKLPAPROP
) muds
where (isnull(service_bin_type,0) != isnull(rate_bin_type,0)
